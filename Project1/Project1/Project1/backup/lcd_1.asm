...
LCD_1_Port:           equ    PRT0DR
LCD_1_PortMode0:      equ    PRT0DM0
LCD_1_PortMode1:      equ    PRT0DM1

LCD_1_E:              equ    10h
LCD_1_RW:             equ    40h
LCD_1_RS:             equ    20h

LCD_1_DATA_MASK:      equ    0Fh
LCD_1_READY_BIT:      equ    08h

LCD_1_DATA_READ:      equ    ( LCD_1_E | LCD_1_RW | LCD_1_RS )
LCD_1_CNTL_READ:      equ    ( LCD_1_E | LCD_1_RW )
LCD_1_PORT_WRITE:     equ    7Fh
LCD_1_PORT_MASK:      equ    7Fh

LCD_1_DISP_INC:       equ    03h
LCD_1_DISP_OFF:       equ    08h
LCD_1_DISP_ON:        equ    0Ch
LCD_1_4BIT_2LINE:     equ    2Ch
...
LCD_1_BG_CHAR_WIDTH:  equ    16     ; 16 characters in width
LCD_1_BG_SEG_WIDTH:   equ    80     ; 16 * 5 = 80
LCD_1_BG_COL_START:   equ     0     ; Always start in the left most column

                                  ; Offsets for 2x16, 2x20, 4x20
                                  ; Change these values for a custom LCD

LCD_1_ROW1_OFFSET:    equ    80h    ; Address/command offset for row 1
LCD_1_ROW2_OFFSET:    equ    C0h    ; Address/command offset for row 2
LCD_1_ROW3_OFFSET:    equ    94h    ; Address/command offset for row 1
LCD_1_ROW4_OFFSET:    equ    D4h    ; Address/command offset for row 2

LCD_1_BG_ROW1_OFFSET: equ    80h    ; Address/command offset for row 1
LCD_1_BG_ROW2_OFFSET: equ    C0h    ; Address/command offset for row 2

LCD_1_CG_RAM_OFFSET:  equ    40h    ; Offset to character RAM
...
_LCD_1_PrHexByte:
    ...
    index LCD_1_HEX_STR                  ; Get Hex value
...
;
 LCD_1_WriteData:
_LCD_1_WriteData:
 LCD_1_Write_Data:   ; Do not use
_LCD_1_Write_Data:   ; Do not use
    RAM_PROLOGUE RAM_USE_CLASS_1
    call  LCD_1_Check_Ready            ; Make sure controller is ready
                                       ; A is preserved in LCD_1_Check_Ready
    ...
    call  LCD_1_WDATA_Nibble           ; Write Upper nibble
    pop   A                            ; Retrieve copy of character
    and   A,0Fh                        ; Mask off high nibble
    nop
    nop
    nop
    call  LCD_1_WDATA_Nibble   ; Write Lower nibble
    RAM_EPILOGUE RAM_USE_CLASS_1
    ret
.ENDSECTION
...
_LCD_1_Control:
    ...
    call  LCD_1_WCNTL_Nibble           ; Write high nibble
    pop   A                            ; Restore copy of byte
    and   A,0Fh                        ; Mask off high nibble
    nop
    nop
    nop
    call  LCD_1_WCNTL_Nibble             ; Write Lower nibble
    RAM_EPILOGUE RAM_USE_CLASS_1
    ret
.ENDSECTION
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: LCD_1_WCNTL_Nibble
;
;  DESCRIPTION:
...
;
 LCD_1_WCNTL_Nibble:
    ...
    push  A
    RAM_SETPAGE_CUR >NO_SHADOW                   ; Set CUR_PP to LCD variable address
    and   [NO_SHADOW],~LCD_1_PORT_MASK
    mov   A,[NO_SHADOW]
    mov   reg[LCD_1_Port],A                      ; Reset control lines

    pop   A
    and   A,LCD_1_DATA_MASK                      ; Make sure no bogus data in MSN
    or    A,LCD_1_E                              ; Bring "E" Enable line high
    or    A,[NO_SHADOW]                          ; OR in bit 7 just
    mov   reg[LCD_1_Port], A                     ; Write data
    mov   [NO_SHADOW],A                          ; Keep shadow register in sync
    nop
    and   A,(~LCD_1_PORT_MASK|LCD_1_DATA_MASK)   ; Disable E signal and leave data on bus.
    mov   [NO_SHADOW],A                          ; Keep shadow register in sync
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: LCD_1_WDATA_Nibble
;
;  DESCRIPTION:
...
LCD_1_WDATA_Nibble:
    ...
    RAM_SETPAGE_CUR >NO_SHADOW          ; Set CUR_PP to LCD variable address
    and   [NO_SHADOW],~LCD_1_PORT_MASK
    or    [NO_SHADOW],LCD_1_RS                        ; Raise RS to signify a Data Write
    mov   A,[NO_SHADOW]
    mov   reg[LCD_1_Port],A

    pop   A
    and   A,LCD_1_DATA_MASK                           ; Make sure no bogus data in A[7:4]
    or    A,(LCD_1_E | LCD_1_RS)                      ; Bring "E" Enable line high
    or    A,[NO_SHADOW]                               ; Keep shadow in sync
    mov   reg[LCD_1_Port], A                            ; Write data
    mov   [NO_SHADOW],A                               ; Keep shadow in sync
    NOP
    and   A,(~LCD_1_PORT_MASK|LCD_1_DATA_MASK|LCD_1_RS)  ; Disable E signal and leave Data on bus
    mov   [NO_SHADOW],A                               ; keep shadow in sync
    mov   reg[LCD_1_Port],A
    RAM_EPILOGUE RAM_USE_CLASS_4
    ret
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: LCD_1_Check_Ready
;
;  DESCRIPTION:
...
LCD_1_Check_Ready:
    ...
    RAM_SETPAGE_CUR >NO_SHADOW                        ; Set CUR_PP to LCD variable address
    and   [NO_SHADOW],~LCD_1_PORT_MASK                ; Mask of all LCD bits
    mov   A,[NO_SHADOW]
    mov   reg[LCD_1_Port],A                           ; Zero LCD port bits

    and   [NO_SHADOW],~LCD_1_DATA_MASK                ; Clear out LCD mode bits.
    mov   A,[NO_SHADOW]
    M8C_SetBank1                                      ; Change port mode to read status
    mov   reg[LCD_1_PortMode0],A                      ; Setup LCD Port for reading
    M8C_SetBank0

    or    [NO_SHADOW],LCD_1_RW                        ; Raise RW to signify Read operation
    mov   A,[NO_SHADOW]
    mov   reg[LCD_1_Port],A
    NOP
...
.LCD_RDY_LOOP:
    or    [NO_SHADOW], LCD_1_CNTL_READ                          ; Raise E to start cycle
    mov   A,[NO_SHADOW]
    mov   reg[LCD_1_Port],A

    nop                                               ; Wait 2 nops to make sure data is ready
    nop
    mov   A,reg[LCD_1_Port]
...
ENDIF   
    ...
    push  A
    and   [NO_SHADOW],(~LCD_1_PORT_MASK | LCD_1_RW)             ; Lower E signal
    mov   A,[NO_SHADOW]
    mov   reg[LCD_1_Port],A
    ...
    pop   A
    and   A,LCD_1_READY_BIT                                       ; Check busy
...
.UNLOCK:
    ...
    or    [NO_SHADOW],LCD_1_PORT_WRITE                ; Revert Data bit to Write mode
    mov   A,[NO_SHADOW]
    M8C_SetBank1
    mov   reg[LCD_1_PortMode0],A                      ; Setup LCD Port for writing
    M8C_SetBank0
    pop   A
...
_LCD_1_Init:
    ...
    and   [NO_SHADOW],~LCD_1_PORT_MASK                          ; Mask off LCD bits
    or    [NO_SHADOW],LCD_1_PORT_WRITE                          ; Set LCD port for writing
    and   [NO_SHADOW],~LCD_1_PORT_MASK                          ; Mask off LCD bits
    ...
    mov   A,LCD_1_4BIT_2LINE                  ; Setup for 4 bit interface, 2 line
    call  LCD_1_Control

    mov   A,LCD_1_DISP_OFF
    call  LCD_1_Control

    mov   A,LCD_1_DISP_ON
    call  LCD_1_Control

    mov   A,LCD_1_DISP_INC
    call  LCD_1_Control
...
.LITERAL
LCD_1_ROW_OFFSET::
     DB    LCD_1_ROW1_OFFSET, LCD_1_ROW2_OFFSET, LCD_1_ROW3_OFFSET, LCD_1_ROW4_OFFSET
.ENDLITERAL
...
_LCD_1_Position:
    ...
    push  X                            ; Store COL
    index LCD_1_ROW_OFFSET ; Get ROW memory offset from table
...
.SECTION
 ...
 _LCD_1_Delay50u:                      ; [11]  Call
    ...
    cmp   A,05h
    jnc   .Delay50u_End
    index LCD_1_Delay50u_Table ; [13] Get delay value
.Delay50u_Loop:                          ;
    dec   A                              ; [4]
    jnz   .Delay50u_Loop                 ; [5]
.Delay50u_End:
...
.VBG_LOOPX:
    push  A
    index LCD_1_ROW_OFFSET  ; Get row offset
...
_LCD_1_InitVBG:
    ...
    mov   A,LCD_1_CG_RAM_OFFSET              ; Setup pointer
...
.VBG_Load:
    ...
    pop  A
    pop  A
    mov  A,LCD_1_DISP_ON                    ; Turn on display, don't really
...
_LCD_1_InitBG:
 ...
 .BG_Loop2:
    ...
    jnz   .BG_OTHER
    index LCD_1_BG_TYPE1
    jmp   .BG_Load
 .BG_OTHER:
    index LCD_1_BG_TYPE2
 .BG_Load:
